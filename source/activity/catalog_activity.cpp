            ...
            progressDialog->close([this, aliveFlag, latestVersion, result] {
                ...
                if (!result.ok) {
                    brls::Dialog* errorDialog =
                        new brls::Dialog("Falha ao instalar a atualização.\n\n" + result.error);
                    errorDialog->addButton("Fechar", [] {});
                    errorDialog->open();
                    return;
                }

                brls::Dialog* doneDialog =
                    new brls::Dialog("Atualizado para " + latestVersion + "!\n\nFeche o app e abra de novo pra usar a nova versão.");
