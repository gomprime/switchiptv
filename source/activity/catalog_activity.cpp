...
void CatalogActivity::performUpdateDownload(const std::string& downloadUrl, const std::string& latestVersion) {
    // A caixa do diálogo tem 720px de largura FIXA (definido na própria
    // borealis, `dialogXML`, compartilhado por todo diálogo do app —
    // "Sobre", erros, etc.) — não dá pra encolher isso por fora sem
    // patchear a lib inteira. Um conteúdo pequeno só deixa a caixa
    // parecendo vazia/grande por comparação; o jeito certo é preencher
    // bem o espaço disponível em vez de tentar encolher a caixa.
    constexpr float kContentWidth = 640;
    ...
    progressBox->setPadding(40);
    progressBox->setWidth(kContentWidth);

    brls::Label* label = new brls::Label();
    label->setText("Baixando atualização...");
    label->setFontSize(22);
    label->setMarginBottom(20);
    progressBox->addView(label);

    constexpr float kBarWidth = kContentWidth - 80;
    brls::Box* barBox = new brls::Box();
    barBox->setWidth(kBarWidth);
    barBox->setHeight(10);
    barBox->setMarginBottom(14);

    brls::Rectangle* barBg = new brls::Rectangle(nvgRGBA(255, 255, 255, 60));
    barBg->setGrow(1.0f);
    barBg->setHeight(10);
    barBg->setCornerRadius(5);
    barBox->addView(barBg);

    brls::Rectangle* barFill = new brls::Rectangle(nvgRGB(2, 177, 244));
    barFill->setPositionType(brls::PositionType::ABSOLUTE);
    barFill->setPositionTop(0);
    barFill->setPositionLeft(0);
    barFill->setHeight(10);
    barFill->setWidth(0);
    barFill->setCornerRadius(5);
    barBox->addView(barFill);
    progressBox->addView(barBox);

    brls::Label* percentLabel = new brls::Label();
    percentLabel->setFontSize(18);
    percentLabel->setText("0%");
    progressBox->addView(percentLabel);
    ...
    auto aliveFlag = alive;
    brls::async([this, aliveFlag, downloadUrl, latestVersion, progressDialog, barFill, percentLabel]() {
        auto onProgress = [aliveFlag, barFill, percentLabel](double downloaded, double total) {
            if (total <= 0) return;
            float fraction = static_cast<float>(downloaded / total);
            if (fraction > 1.0f) fraction = 1.0f;
            brls::sync([aliveFlag, barFill, percentLabel, fraction] {
                if (!*aliveFlag) return;
                barFill->setWidth(kBarWidth * fraction);
                percentLabel->setText(std::to_string(static_cast<int>(fraction * 100 + 0.5f)) + "%");
            });
        };
        iptv::UpdateInstallResult result = iptv::downloadAndInstall(downloadUrl, onProgress);
        brls::sync([this, aliveFlag, latestVersion, progressDialog, result]() {
            progressDialog->close([this, aliveFlag, latestVersion, result] {
                ...
                // pode se auto-substituir enquanto está rodando. Só um
                // botão de propósito: no Switch, apertar HOME apenas
                // SUSPENDE o app (não fecha de verdade) — se o usuário
                // escolhesse "depois" e voltasse pelo HOME, essa troca
                // nunca chegaria a rodar (confirmado com usuário real:
                // "não instalou" na primeira tentativa, exatamente por
                // isso). Forçar o fechamento de verdade aqui evita essa
                // armadilha.
                brls::Dialog* doneDialog = new brls::Dialog(
                    "Versão " + latestVersion +
                    " baixada!\n\nO app vai fechar agora pra instalar. Abra de novo pelo Homebrew Launcher "
                    "(não pelo botão HOME — ele só suspende, não conta como reabrir).");
