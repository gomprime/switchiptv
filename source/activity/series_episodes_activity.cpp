...
brls::View* SeriesEpisodesActivity::createContentView() {
    ...
    brls::Box* statusRow = new brls::Box();
    statusRow->setAxis(brls::Axis::ROW);
    statusRow->setAlignItems(brls::AlignItems::CENTER);

    statusSpinner = new brls::ProgressSpinner();
    statusSpinner->setWidth(20);
    statusSpinner->setHeight(20);
    statusSpinner->setMarginRight(10);
...
void SeriesEpisodesActivity::loadEpisodes() {
    if (series.streamId == 0 || auth.mode != iptv::AuthMode::Xtream) {
        ...
        statusSpinner->setVisibility(brls::Visibility::GONE);
        return;
    }
