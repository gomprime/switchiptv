...
CatalogContentView::CatalogContentView(iptv::StoredAuth auth, iptv::ContentTab tab,
    ...
    // Logo (mesma usada na tela de login) — dá identidade de marca também
    // depois do login, não só na primeira tela que o usuário vê.
    brls::Image* logo = new brls::Image();
    logo->setWidth(brls::View::AUTO);
    logo->setHeight(40);
    logo->setMarginRight(16);
    ...
    brls::Box* categoriesStatusRow = new brls::Box();
    categoriesStatusRow->setAxis(brls::Axis::ROW);
    categoriesStatusRow->setAlignItems(brls::AlignItems::CENTER);
    categoriesStatusRow->setMargins(10, 10, 0, 10);

    categoriesSpinner = new brls::ProgressSpinner();
    categoriesSpinner->setWidth(20);
    categoriesSpinner->setHeight(20);
    categoriesSpinner->setMarginRight(10);
    categoriesStatusRow->addView(categoriesSpinner);

    categoriesStatusLabel = new brls::Label();
    categoriesStatusLabel->setFontSize(16);
    categoriesStatusLabel->setText("Carregando...");
    categoriesStatusRow->addView(categoriesStatusLabel);
    categoriesBox->addView(categoriesStatusRow);

    categoriesRecycler = new brls::RecyclerFrame();
    categoriesRecycler->setGrow(1.0f);
    // A borealis desenha a barra de rolagem ~14-18px da borda direita — sem
    // esse respiro, o destaque de foco (que ignora recorte/scissor de
    // propósito, ver View::drawHighlight) fica desenhado por cima dela.
    categoriesRecycler->setPaddingRight(20);
    ...
    brls::Box* itemsStatusRow = new brls::Box();
    itemsStatusRow->setAxis(brls::Axis::ROW);
    itemsStatusRow->setAlignItems(brls::AlignItems::CENTER);

    itemsSpinner = new brls::ProgressSpinner();
    itemsSpinner->setWidth(24);
    itemsSpinner->setHeight(24);
    itemsSpinner->setMarginRight(12);
    itemsSpinner->setVisibility(brls::Visibility::GONE);
    itemsStatusRow->addView(itemsSpinner);

    itemsStatusLabel = new brls::Label();
    itemsStatusLabel->setFontSize(20);
    itemsStatusRow->addView(itemsStatusLabel);
    itemsBox->addView(itemsStatusRow);

    itemsRecycler = new brls::RecyclerFrame();
    itemsRecycler->setGrow(1.0f);
    itemsRecycler->setPaddingRight(20);
    ...
    if (isM3u) {
        ...
        brls::async([ASYNC_TOKEN, m3uUrl]() {
            ...
            brls::sync([ASYNC_TOKEN, result]() {
                ASYNC_RELEASE
                categoriesSpinner->setVisibility(brls::Visibility::GONE);
                if (!result.ok) {
...
void CatalogContentView::selectCategory(size_t index) {
    ...
    setItemsStatus("Carregando...", true);
    iptv::Credentials creds = auth.credentials;
    iptv::ContentTab contentTab = tab;
