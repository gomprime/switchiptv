...
constexpr float kProgressBarHeight = 4;
...
PosterRowCell::PosterRowCell() {
    ...
    for (int i = 0; i < kColumns; i++) {
        ...
        // Cartão: fundo sutil + cantos arredondados (o fallback "TV" fica
        // com cara de placeholder em vez de texto solto na tela).
        slot.imageBox->setCornerRadius(10);
        slot.imageBox->setBackgroundColor(nvgRGBA(255, 255, 255, 18));
        ...
        // Acompanha o arredondamento do cartão — sem isso os cantos retos
        // da faixa do nome "vazam" por cima dos cantos arredondados.
        slot.nameLabel->setCornerRadius(10);
        ...
        slot.favoriteLabel->setPositionTop(4);
        slot.favoriteLabel->setPositionRight(4);
        slot.favoriteLabel->setText(" ★ ");
        slot.favoriteLabel->setFontSize(20);
        slot.favoriteLabel->setTextColor(nvgRGB(255, 215, 0));
        // Chip com fundo escuro atrás da estrela — sem isso ela some em
        // pôsteres claros.
        slot.favoriteLabel->setBackgroundColor(nvgRGBA(0, 0, 0, 140));
        slot.favoriteLabel->setCornerRadius(8);
        slot.favoriteLabel->setVisibility(brls::Visibility::GONE);
        slot.imageBox->addView(slot.favoriteLabel);

        // Selo de "assistido" — canto superior ESQUERDO (favorito já ocupa
        // o direito). Bolinha verde em vez de um "✓" de texto: a fonte
        // embutida no app não tem esse glifo (aparecia um quadrado de
        // caractere não encontrado) — forma geométrica não depende de fonte
        // nenhuma.
        slot.watchedBadge = new brls::Box();
        slot.watchedBadge->setPositionType(brls::PositionType::ABSOLUTE);
        slot.watchedBadge->setPositionTop(4);
        slot.watchedBadge->setPositionLeft(4);
        slot.watchedBadge->setWidth(20);
        slot.watchedBadge->setHeight(20);
        slot.watchedBadge->setAlignItems(brls::AlignItems::CENTER);
        slot.watchedBadge->setJustifyContent(brls::JustifyContent::CENTER);
        slot.watchedBadge->setBackgroundColor(nvgRGBA(0, 0, 0, 140));
        slot.watchedBadge->setCornerRadius(10);
        slot.watchedBadge->setVisibility(brls::Visibility::GONE);

        brls::Rectangle* watchedDot = new brls::Rectangle(nvgRGB(74, 222, 128));
        watchedDot->setWidth(10);
        watchedDot->setHeight(10);
        watchedDot->setCornerRadius(5);
        slot.watchedBadge->addView(watchedDot);

        slot.imageBox->addView(slot.watchedBadge);

        // Barra de progresso: trilho translúcido + preenchimento amarelo
        // (mesma cor do estado "em andamento" do Android), só visível em
        // item parcialmente assistido. Largura do preenchimento sempre em
        // pixels — NUNCA setWidthPercentage aqui (ver aviso da imagem
        // acima sobre recursão no Yoga).
        slot.progressBarBg = new brls::Rectangle(nvgRGBA(255, 255, 255, 60));
        slot.progressBarBg->setPositionType(brls::PositionType::ABSOLUTE);
        slot.progressBarBg->setPositionTop(kImageHeight - kProgressBarHeight);
        slot.progressBarBg->setPositionLeft(0);
        slot.progressBarBg->setWidth(kImageWidth);
        slot.progressBarBg->setHeight(kProgressBarHeight);
        slot.progressBarBg->setVisibility(brls::Visibility::GONE);
        slot.imageBox->addView(slot.progressBarBg);

        slot.progressBarFill = new brls::Rectangle(nvgRGB(250, 204, 21));
        slot.progressBarFill->setPositionType(brls::PositionType::ABSOLUTE);
        slot.progressBarFill->setPositionTop(kImageHeight - kProgressBarHeight);
        slot.progressBarFill->setPositionLeft(0);
        slot.progressBarFill->setHeight(kProgressBarHeight);
        slot.progressBarFill->setVisibility(brls::Visibility::GONE);
        slot.imageBox->addView(slot.progressBarFill);
...
void PosterRowCell::setItems(const std::vector<iptv::CatalogItem>& itemsInRow,
    ...
    for (int i = 0; i < kColumns; i++) {
        ...
        // Mesmo critério do Android (>=90% assistido = concluído): abaixo
        // disso mas com alguma posição salva, mostra a barra de progresso;
        // frações minúsculas (poucos segundos, o usuário nem chegou a
        // assistir de verdade) não mostram nada.
        auto progress = iptv::getProgress(item.id);
        bool watched = false;
        bool inProgress = false;
        float fraction = 0.0f;
        if (progress.has_value() && progress->durationSeconds > 0) {
            fraction = static_cast<float>(progress->positionSeconds / progress->durationSeconds);
            watched = fraction >= 0.9f;
            inProgress = !watched && fraction > 0.02f;
        }
        slot.watchedBadge->setVisibility(watched ? brls::Visibility::VISIBLE : brls::Visibility::GONE);
        slot.progressBarBg->setVisibility(inProgress ? brls::Visibility::VISIBLE : brls::Visibility::GONE);
        slot.progressBarFill->setVisibility(inProgress ? brls::Visibility::VISIBLE : brls::Visibility::GONE);
        if (inProgress) {
            slot.progressBarFill->setWidth(kImageWidth * std::min(fraction, 1.0f));
        }
